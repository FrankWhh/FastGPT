...
export function setThemeContent(data) {
  themeContent = data;
}
function tXml(S) {
  'use strict';
...
export function getContentTypes(zip) {
...
export function getSlideSize(zip) {
...
export function processSingleSlide(zip, sldFileName, index, slideSize) {
  // self.postMessage({
  //     "type": "INFO",
...
function getShapeFill(node, isSvgMode) {
  ...
  if (fillColor === undefined) {
    var schemeClr =
      'a:' + getTextByPathList(node, ['p:spPr', 'a:solidFill', 'a:schemeClr', 'attrs', 'val']);
  ...
  if (fillColor === undefined) {
    var schemeClr =
      'a:' + getTextByPathList(node, ['p:style', 'a:fillRef', 'a:schemeClr', 'attrs', 'val']);
    fillColor = getSchemeColorFromTheme(schemeClr);
  }
...
export function processMsgQueue() {
  for (var i = 0; i < MsgQueue.length; i++) {
    processSingleMsg(MsgQueue[i].data);
  }
}
